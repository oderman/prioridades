...
$idioma = $pdo->prepare("SET lc_time_names = 'es_ES'");
$idioma->execute();

$consulta = $pdo->prepare("SELECT rev_id AS id, rev_titulo AS title, rev_descripcion AS description, CONCAT('https://revistaprioridades.com/admin/img/portadas/', rev_portada) AS image, UPPER(MONTHNAME(rev_publicacion)) AS month, YEAR(rev_publicacion) AS year FROM revistas
